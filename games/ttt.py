...
def player_move(r, c):
    global player_turn, board, buttons, turn_label, last_move
    ...
    if board[r][c] == "":
        ...
        # Save last move for the delete functionality
        last_move = (r, c)
...
def reset_board():
    global board, buttons, player_turn, turn_label, last_move
    ...
    player_turn = "X"
    turn_label.config(text=f"Player {player_turn}'s Turn")
    last_move = None  # Reset the last move

# Function to restart the game
def restart_game():
    reset_board()  # Call reset function to clear the board

# Function to delete the last move
def delete_last_move():
    global player_turn, board, buttons, last_move
    if last_move:
        r, c = last_move
        board[r][c] = ""  # Clear the last move
        buttons[r][c]["text"] = ""  # Clear the button text
        buttons[r][c]["bg"] = "#b2dfdb"  # Reset the button background
        player_turn = "X" if player_turn == "O" else "O"  # Switch turns back
        turn_label.config(text=f"Player {player_turn}'s Turn")
...
# Add a Delete button to change the last response
delete_button = tk.Button(window, text="Delete Last Move", font=('Arial', 14, 'bold'), bg="#ff9800", 
                           fg="white", command=delete_last_move)
